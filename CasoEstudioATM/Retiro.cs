...
public class Retiro : Transaccion
{
    ...
    //constante que corresponde a la opción del menú para cancelar
    private const int CANCELO = 6;

    //constructor de cinco parámetros
    public Retiro(int numeroCuentaUsuario, Pantalla pantallaATM, BaseDatosBanco baseDatosATM, Teclado tecladoATM, DispensadorEfectivo dispensadorEfectivoATM)
        : base(numeroCuentaUsuario, pantallaATM, baseDatosATM)
    {
        //inicializa las referncias a teclado y al dispensador de efectivo
        teclado = tecladoATM;
        ...
        do
        {
            ...
            if (seleccion != CANCELO)
            {
                ...
                if (monto <= saldoDisponible)
                {
                    ...
                    PantallaUsuario.MostrarLineaMensaje("\nNo hay suficiente efectivo disponible en su cuenta.\n\nPor favlo, elija un monto más pequeño.");
            }//fin del if más externo
            else
            {
                PantallaUsuario.MostrarLineaMensaje("\nCancelando operación...");
                transaccionCanelada = true;//el usuario cancela la transacción
            }
        } while ((!efectivoDispensado) && (!transaccionCanelada));
    }//fin del método Ejecutar

    //muestra un menú de pontos apra retirar y la opción para cancelar
    ...
    private int MostrarMenuMontos()
    {
        ...
        //vector de montos que corresponden a los números del menú
        ...
        while(eleccionUsuario == 0)
        {
            ...
            switch(entrada)
            {
                //si el usuario eligió un monto de retiro, devuelve el monto correspondiente del vector de montos
                ...
                case CANCELO: //el usuario eligió cancelar
                    ...
                    break;
                default:
                    PantallaUsuario.MostrarLineaMensaje("Selección inválida. Intente de nuevo");
                    break;
            }//fin del swtich
        }//fin del while
